...
\acknowledgments{ 
I am grateful to Cagri Cetin, Dr. Jay Ligatti, and Dr. Dmitry Goldgof for their precious and constant help on this project.
I would like to thank Glenn Whichard, William Munnerlyn, Sarah Murphy, and Nair Gayatry, for their help through the patent application process.
I also would like to thank Ian Markwood and Benjamin Dunn for their help with proofreading and revisions.
}
...
\label{flow-spec-ex}
\centering
    \includegraphics[width=0.6\textwidth]{try}
...
\chapter{Flow-directed Trace Interpretation}
...
\]
\chapter{Trace Analysis With Partial Observability}
...
Another register transaction level SoC model is constructed using VHDL language. As showed in Fig.~\ref{rtlstruc}, this model consists of two CPU model with instruct set hard coded in, each with its own 1KB cache. The caches are connected to a 4MG memory through a memory bus model. At each rising clock cycle, our model will record selected signal value into a vcd(value change dump) format file.

This model has 6 protocols implemented. It's based on the protocols provided by GEM5 with some modification. 3 types of protocols: read, write and write back is implemented for both CPU. Different from GEM5, the cache miss and cache hit scenarios for both read(/write) are combined into a single protocol of read(/write). Write back protocol is new and will be invoked when Cache need to flush back dirty datas.

We wrote a random test generator inside each CPU. For every clock cycle, each CPU will randomly generate a read or write operation to a random generated address. In order to better active and cache coherent protocol, only first 3 bits of the 16 bits request address are random generated, while the rest of the bits are predefined.
...
\chapter{Flow Specifications and Protocols}
\section{GEM5}

\subsection{write flow specification}
...
\begin{minipage}{.5\textwidth}
 \includegraphics[width=3.0In]{figures/Fig5.png}
 {\tiny
 \[
 \begin{array}{llll}
 ...
 \end{array}
 \]}
 \caption{\footnotesize Flow specification of a cache coherent write operation initiated from CPU1. This flow is symmetric for CPU2. }
 \label{write-flow}
 \end{minipage}%
 ...
 \centering
  \includegraphics[width=2in]{figures/Fih6.png}
  \centering
 {\tiny
 \[
 \begin{array}{llll}
 ...
 \end{array}
 \]}
 \caption{\footnotesize Flow specification of a cache coherent read operation initiated from CPU1 to instruction cache.This flow is symmetric for CPU2. }
 \label{read-flow}
\end{minipage}
 ...
 \subsection{read flow specification}
...
Since instruction cache and data cache have different coherence protocol, there are two flow specifications for read operation generated by CPU1. The LPN specification as shown in Fig.~\ref{read-flow} captures the system flow where CPU1 initiate a memory read to instruction cache. And Fig.~\ref{read-dcache} specifies read operation to data cache from CPU1.
 ...
 \begin{figure} 
 \centerline{
 \includegraphics[width=2.4in]{figures/readDcache.png}}
 ...
 \end{array}
 \]}
 \caption{\footnotesize Flow specification of a cache coherent read operation initiated from CPU1 to  data cache. This flow is symmetric for CPU2. }
 \label{read-dcache}
...
\centering
 ...
 \end{array}
 \]}
 \caption{\footnotesize Flow specification of failed read. This flow is symmetric for CPU2. }
 \label{storefail}
